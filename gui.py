...
class MaxLength(enum.Enum):
    ...
    ROUTES    = "Routes"
    PORT      = "Reclamation & Disposal Orinth ()"
    TRAVEL    = "00 minutes"
...
class GUI():
    def __init__(self, a_ship_data):
        ...
        self.refresh_button.grid(row=4, column=0, pady=5)

        self.calculate_button = ttk.Button(config_frame, text="Calculate",
                                           command=self.calculate_clicked,
                                           state="disabled")
        self.calculate_button.grid(row=4, column=1, pady=5)
    ...
    def calculate_clicked(self):
        ...
        for commodity_name, commodity in self.commodities_dict.items():
            commodity.build_routes(units, profit_per_unit,
                                   buy_time, sell_time)
        ...
        for commodity_name in commodities_with_good_routes:
            ...
            investment = \
                self.commodities_dict[commodity_name].\
                routes[0].purchase_port.price * units
    ...
    def build_commodities_dict(self):
        ...
        commodity_lists = soup.find_all(
            class_="ui inverted fluid card commodity")

        rows_to_process = list()

        for list_ in commodity_lists:
            commodity_category = \
                list_.find(class_="header commodity").string.strip()

            commodity_rows = list_.find_all(class_="item")
            for commodity_row in commodity_rows:
                row_data = [commodity_row, commodity_category]
                rows_to_process.append(row_data)

        threadpool = ThreadPoolExecutor(max_workers=os.cpu_count())
        commodities_list = list()
        for row in rows_to_process:
            threadpool.submit(self.commodity_builder, row, commodities_list)
        threadpool.shutdown(wait=False)

        while len(commodities_list) < len(rows_to_process):
            progress = (len(commodities_list) / len(rows_to_process)) * 100
